<?php
    ...
    if ($uuidQuery) {
        ...
        $sql = "SELECT * FROM transactions WHERE uuid = '$uuid' ";
        ...
        $get_data_query = $conn -> query($sql) or die($conn -> error($conn));
            ...
            while($r = $get_data_query -> fetch_array(MYSQLI_ASSOC)){
                extract($r);
                $result[] = array("uuid" => $uuid, "title" => $title, "payDate" => $payDate, "amount" => $amount, "description" => $description);
            }
            $json = array("status" => 1, "info" => $result);
        }
        else{
            $json = array("status" => 0, "error" => "Transactions not found!");
        }
    }
    else {
